...
function attachment_page_image( $id ) {
    ...
    if ($fileExt == 'svg' || $fileExt == 'svgz') { // SVG Images
        ...
        return '<script defer type="module" src="https://ajax.googleapis.com/ajax/libs/model-viewer/3.4.0/model-viewer.min.js"></script><model-viewer id="Model3D" class="aligncenter" itemprop="image" style="height:380px;min-width:400px" src="' . wp_get_attachment_url($id) . '" alt="' . wp_get_attachment_caption($id) . '" ar poster="" shadow-intensity="0" exposure="1.1" shadow-softness="0" tone-mapping="commerce" camera-controls touch-action="pan-y"></model-viewer>';
...
{"@context":"https://schema.org/","@type":"<?=get_option('site_business');?>","@id":"<?=home_url();?>#<?=get_option('site_representation');?>","name":"<?=bloginfo('name');?>","url":"<?=home_url();?>","contactPoint":{"@type":"ContactPoint","contactType":"<?=addslashes(get_option('phone_type'));?>","telephone":"<?=addslashes(get_option('contact_phone'));?>","url":"<?=home_url();?>/contact/"}},

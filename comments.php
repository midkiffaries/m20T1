...
<section class="comment-form">
    ...
    <?php if (is_user_logged_in()) : // If user is logged in ?>
		<p>You are logged in as <a href="<?=get_option('siteurl');?>/wp-admin/profile.php"><b><?=$user_identity; ?></b></a> | <a href="<?=wp_logout_url(get_permalink()); ?>" class="logout-link">Log out</a></p>
    ...
    <?php else : // If user is not logged in ?>
		...
		<p><label for="url">Website Link</label><input type="url" name="url" id="url" class="comment-url" maxlength="50" value="<?=esc_attr($comment_author_url);?>" placeholder="https://www.example.com" autocapitalize="none" autocorrect="off" autocomplete="url"></p>
    <?php endif; ?>
...
<?php // List blog post user comments
